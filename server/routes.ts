...
export async function registerRoutes(
  ...
  app.post("/api/shift-counts", async (req, res) => {
    try {
      ...
      if (!isRecount && expectedAmount && parseFloat(countedAmount) !== parseFloat(expectedAmount)) {
        ...
        const alertType = type === "start" ? "start_mismatch" : "end_mismatch";
      ...
      for (const loc of mappedLocations) {
        ...
        for (const [blvdStaffId, staffData] of staffMap) {
          ...
          staffStatuses.push({
            ...
            startCount: startRow ? { time: new Date(startRow.created_at).toISOString(), amount: startRow.counted_amount } : null,
            endCount: endRow ? { time: new Date(endRow.created_at).toISOString(), amount: endRow.counted_amount } : null,
